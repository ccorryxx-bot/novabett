<template>
  <div class="min-h-screen bg-black text-white flex flex-col selection:bg-yellow-500/30">
    <!-- Main Content (pushed to bottom for scrolling) -->
    <div class="flex-1">
      <!-- Top Bar: Jackpot + Brand + User Area -->
      <div class="px-4 pt-4 pb-2 space-y-4">
        
        <!-- Jackpot (compact pill style) -->
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <template v-if="isLoggedIn">
              <div class="flex items-center gap-3 bg-[#111] border border-white/10 rounded-full px-3 py-1.5 hover:border-white/20 transition-colors cursor-pointer" @click="refreshBalance">
                <div class="text-right">
                ...
                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-amber-600 flex items-center justify-center text-xs font-black text-black shadow-lg">
                  {{ username.charAt(0).toUpperCase() }}
                </div>
              </div>
              <div class="flex gap-1.5">
                <button @click="showDepositModal = true" class="bg-white hover:bg-gray-200 text-black text-[10px] font-bold px-3 py-1.5 rounded-full transition-all active:scale-95 shadow-sm">Deposit</button>
                <button @click="showWithdrawModal = true" class="bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold px-3 py-1.5 rounded-full transition-all active:scale-95 border border-white/10">Withdraw</button>
              </div>
            </template>
      ...
      <!-- Extra space for smooth scrolling test -->
      <div class="h-8"></div>
